...
class TranscriberConfigTests(unittest.TestCase):
    ...
    def test_sensevoice_rich_markers_are_removed(self) -> None:
        text = "<|zh|><|Speech|>👏你好，开始测试。<|SAD|>😔"

        cleaned = transcriber._clean_provider_transcript(text, transcriber.SENSEVOICE_MODEL)

        self.assertEqual(cleaned, "你好，开始测试。")

    def test_non_sensevoice_transcript_is_unchanged(self) -> None:
        text = "keep this 😔"

        cleaned = transcriber._clean_provider_transcript(text, "whisper-large-v3-turbo")

        self.assertEqual(cleaned, text)

    def test_sensevoice_marker_only_result_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            audio = Path(tmp) / "sample.wav"
            audio.write_bytes(b"RIFFtest")
            ...
            result = transcriber._transcribe_openai_compatible_once(
                audio,
                {
                    ...
                    "base_url": "https://asr.example.test/openai/v1",
                    "api_key": "secret-key",
                    "model": transcriber.SENSEVOICE_MODEL,
                    "language": "zh",
                },
                attempt=1,
                opener=lambda request, timeout=None: _FakeResponse(
                    b'{"text":"<|nospeech|><|NEUTRAL|>"}'
                ),
            )

        self.assertFalse(result.success)
        self.assertEqual(result.message, "OpenAI-compatible returned no transcript")
