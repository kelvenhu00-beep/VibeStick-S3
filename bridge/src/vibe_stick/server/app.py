...
class BridgeStateStore:
    ...
    def get_state(self) -> VibeStickState:
        with self._lock:
            self.recording.expire_if_stale()
    ...
    def confirm_recording(self) -> dict[str, Any]:
        session = self.recording.confirm()
        return {"recording": session.to_jsonable(), "state": self.get_state().to_jsonable()}

    def cancel_recording(self) -> dict[str, Any]:
        session = self.recording.cancel()
        return {"recording": session.to_jsonable(), "state": self.get_state().to_jsonable()}
    ...
    class VibeStickHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            if parsed.path == "/event":
                ...
                attach_ms = round((time.monotonic() - attach_started_at) * 1000)
                print(
                ...
                body = self._read_json_body()
                response = store.stop_recording(body)
                self._send_json(
                    _compact_recording_response(response)
                    if _first(parse_qs(parsed.query), "compact") == "1"
                    else response
                )
            elif parsed.path == "/recording/confirm":
                response = store.confirm_recording()
                self._send_json(
                    _compact_recording_response(response)
                    if _first(parse_qs(parsed.query), "compact") == "1"
                    else response
                )
            elif parsed.path == "/recording/cancel":
                response = store.cancel_recording()
                self._send_json(
                    _compact_recording_response(response)
                    if _first(parse_qs(parsed.query), "compact") == "1"
                    else response
...
def run_server(host: str, port: int) -> None:
    ...
    if bonjour.start():
        print(
            "VibeStick Bridge advertised as VibeStick Bridge._vibestick._tcp.local",
            flush=True,
        )
    else:
