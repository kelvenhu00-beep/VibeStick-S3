...
SENSEVOICE_PREFIX_MARKERS = ("🎼", "👏", "😀", "😭", "🤧", "😷", "❓")
SENSEVOICE_SUFFIX_MARKERS = ("😊", "😔", "😡", "😰", "🤢", "😮")
SENSEVOICE_CONTROL_TOKENS = (
    "<|zh|>",
    "<|en|>",
    "<|yue|>",
    "<|ja|>",
...
def _transcribe_openai_compatible_once(
    ...
    try:
        ...
        _log_asr_timing(audio_file, config, attempt, started_at, f"http_{exc.code}")
        return TranscriptionResult(
            success=False,
        ...
        _log_asr_timing(audio_file, config, attempt, started_at, type(exc).__name__)
        return TranscriptionResult(
            success=False,
    ...
    try:
        ...
        _log_asr_timing(audio_file, config, attempt, started_at, "invalid_json")
        return TranscriptionResult(success=False, message=f"{label} returned unreadable JSON", source=source)
    text = str(payload.get("text") or "").strip()
    if not text:
        _log_asr_timing(audio_file, config, attempt, started_at, "empty")
        return TranscriptionResult(success=False, message=f"{label} returned no transcript", source=source)
    text = _clean_provider_transcript(text, config.get("model", ""))
    if not text:
        _log_asr_timing(audio_file, config, attempt, started_at, "markers_only")
        return TranscriptionResult(success=False, message=f"{label} returned no transcript", source=source)
    _log_asr_timing(audio_file, config, attempt, started_at, "success")
    return TranscriptionResult(
        text=text,
        success=True,
...
def _clean_provider_transcript(text: str, model: str) -> str:
    cleaned = text.strip()
    if model != SENSEVOICE_MODEL:
        return cleaned
    for token in SENSEVOICE_CONTROL_TOKENS:
        cleaned = cleaned.replace(token, "")
    cleaned = cleaned.strip()
    while cleaned and any(cleaned.startswith(marker) for marker in SENSEVOICE_PREFIX_MARKERS):
        cleaned = cleaned[1:].lstrip()
    while cleaned and any(cleaned.endswith(marker) for marker in SENSEVOICE_SUFFIX_MARKERS):
...
def _log_asr_timing(
    ...
    print(
        "asr request "
        f"model={config.get('model') or DEFAULT_ASR_MODEL} "
        f"attempt={attempt} "
