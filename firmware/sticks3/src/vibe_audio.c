...
#define AUDIO_STARTUP_DISCARD_MS 40
#define AUDIO_MAX_SECONDS 60
...
static void audio_task(void *arg)
{
    ...
    size_t dropped = 0;
    size_t startup_bytes_remaining =
        (VIBE_STICK_AUDIO_SAMPLE_RATE * VIBE_STICK_AUDIO_CHANNELS *
         (VIBE_STICK_AUDIO_BITS_PER_SAMPLE / 8) * AUDIO_STARTUP_DISCARD_MS) / 1000;
    ...
    while (atomic_load(&s_running)) {
        ...
        size_t frame_bytes_len = sizeof(frame);
        if (startup_bytes_remaining > 0) {
            size_t discard_bytes = startup_bytes_remaining < frame_bytes_len
                ? startup_bytes_remaining : frame_bytes_len;
            frame_bytes += discard_bytes;
            frame_bytes_len -= discard_bytes;
            startup_bytes_remaining -= discard_bytes;
        }
        if (frame_bytes_len == 0) {
            continue;
        }
        if (s_audio_len + frame_bytes_len <= s_audio_capacity) {
            memcpy(s_audio_buffer + s_audio_len, frame_bytes, frame_bytes_len);
            s_audio_len += frame_bytes_len;
        } else {
            dropped += frame_bytes_len;
        }
    }
