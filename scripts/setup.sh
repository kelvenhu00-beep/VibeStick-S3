...
if [ -n "$lan_ip" ]; then
  printf '%s\n' "Detected Mac LAN IP on en0 for compatibility fallback: $lan_ip"
  ...
  if is_placeholder_host "$bridge_host"; then
    ...
    printf '%s\n' "Set the compatibility fallback host; Bonjour discovery is preferred at runtime."
  elif [ "$bridge_host" = "$lan_ip" ]; then
    printf '%s\n' "Compatibility fallback already matches the detected Mac LAN IP."
  else
    printf '%s\n' "Kept existing compatibility fallback ($bridge_host); Bonjour discovery handles address changes."
  fi
else
  printf '%s\n' "WARN: Could not set a fallback address; Bonjour discovery can still locate the bridge."
fi
