...
check_secrets() {
  ...
  if is_placeholder_host "$bridge_host"; then
    warn "VIBE_STICK_BRIDGE_HOST fallback is not usable; Bonjour discovery is required."
  else
    pass "VIBE_STICK_BRIDGE_HOST compatibility fallback is configured."
  fi
}
...
check_usb_runtime() {
  usb_port=""
  for path in /dev/cu.usbmodem*; do
    if [ -e "$path" ]; then
      usb_port="$path"
      break
    fi
  ...
  if [ -z "$usb_port" ]; then
    warn "No USB modem port is currently connected; Wi-Fi fallback remains available."
    return
  fi
  pass "USB modem port is connected."

  bridge_log="$APP_SUPPORT_DIR/bridge.log"
  if [ -f "$bridge_log" ] &&
     tail -n 200 "$bridge_log" 2>/dev/null | grep -q "bridge GET /state transport=USB result=ESP_OK"; then
    pass "StickS3 state traffic is using the USB runtime transport."
  else
    warn "USB is connected but a successful USB state request was not found in the recent bridge log."
  fi
}
